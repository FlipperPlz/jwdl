...
pub const VTable = struct {
    ...
    computeTitleTextFn: *const fn(*anyopaque, std.mem.Allocator) std.mem.Allocator.Error![]const u8 = Defaults.getStructTitle,
    destroyFn: ?*const fn(*anyopaque, std.mem.Allocator) void,
    setClickPosition: *const fn(*anyopaque, ?vxfw.Point) void = Defaults.setClickPosition
};
...
pub fn createVTable(comptime T: type, comptime custom_vtable: ITab.VTable) ITab.VTable {
    return .{
        ...
        .openFn = if (custom_vtable.openFn != Defaults.open)
            custom_vtable.openFn
        else struct {
                pub fn open(ptr: *anyopaque, point: ?vxfw.Point) void {
                    const itab: *ITab = @ptrCast(@alignCast(ptr));
                    Defaults.open(ptr, point);
                    const self: *T = @ptrCast(@alignCast(itab.ptr));
                    if (@hasDecl(T, "open")) {
                        self.open(point);
                    } else if (@hasDecl(T, "onOpen")) {
                        self.onOpen(point);
                    }
                }
            }.open,
        .closeFn = if (custom_vtable.closeFn != Defaults.close)
            custom_vtable.closeFn
        else struct {
        ...
        .computeTitleTextFn = struct {
            pub fn computeTitleText(_: *anyopaque, arena: std.mem.Allocator) ![]const u8 {
                ...
                return Defaults.titleFromName(arena, name);
            }
        }.computeTitleText,
...
pub const Defaults = struct {
    ...
    pub fn open(ptr: *anyopaque, point: ?vxfw.Point) void {
        const self: *ITab = @ptrCast(@alignCast(ptr));
        self.is_open = true;
        setClickPosition(ptr, point);
    }

    pub fn close(ptr: *anyopaque) void {
        const self: *ITab = @ptrCast(@alignCast(ptr));
        self.is_open = false;
        setClickPosition(ptr, null);
    }
    ...
    pub fn setClickPosition(ptr: *anyopaque, click_pos: ?vxfw.Point) void {
        const self: *ITab = @ptrCast(@alignCast(ptr));
    ...
    pub fn getStructFirstChar(ptr: *anyopaque, arena: std.mem.Allocator) u8 {
        ...
        return if (name.len > 0) std.ascii.toLower(name[0]) else ' ';
    }
