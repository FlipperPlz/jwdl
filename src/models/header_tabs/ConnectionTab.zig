...
pub const vtable = tabs.ITab.createVTable(ConnectionTab, .{
...
pub fn init(allocator: std.mem.Allocator) std.mem.Allocator.Error!*ConnectionTab {
    const t = try allocator.create(ConnectionTab);
...
pub fn deinit(ptr: *anyopaque, allocator: std.mem.Allocator) void {
    const self: *ConnectionTab = @ptrCast(@alignCast(ptr));
